...
impl CDEngine {
    ...
    pub fn detect_poly_collision(&self, shape: &SPolygon, filter: &impl HazardFilter) -> bool {
        self.detect_poly_collision_with_holes(shape, &[], filter)
    }

    /// Like [`Self::detect_poly_collision`] but accounts for the candidate's own holes (inner rings).
    /// A hazard whose representative point lies fully inside one of `shape_holes` is considered
    /// nested in the candidate's pocket and therefore NOT colliding.
    pub fn detect_poly_collision_with_holes(
        &self,
        shape: &SPolygon,
        shape_holes: &[Arc<SPolygon>],
        ...
        if self.bbox().relation_to(shape.bbox) != GeoRelation::Surrounding {
            ...
            // Check for boundary segment intersections with the shape
            for segment in shape.segment_iter() {
                if v_qt_root.collides(&segment, filter).is_some() {
                    return true;
                }
            }
    ...
    /// # Arguments
    /// * `shape` - The candidate shape to be checked for containment
    /// * `shape_holes` - Inner rings (holes) of the candidate shape. Pass `&[]` for shapes
    ///   without holes.
    /// * `haz_shape` - Outer ring of the respective hazard
    /// * `haz_holes` - Inner rings (free pockets) of the hazard. When the candidate sits fully
    ///   inside one of these pockets it does NOT collide with the hazard. Pass `&[]` for hazards
    ///   without holes.
    ...
    pub fn detect_containment_collision(
        ...
        let contained = match haz_to_shape_bbox_relation {
            GeoRelation::Surrounding => {
                // Hazard is the bigger one, candidate sits inside the hazard's bbox.
                // If the candidate's representative point is inside the hazard's outer AND
                // inside one of the hazard's holes, the candidate is nesting in the hole's
                // free pocket -> NOT contained in the forbidden region.
            ...
            GeoRelation::Enclosed => {
                // Candidate is the bigger one, hazard sits inside the candidate's bbox.
                // The hazard's representative point inside one of the candidate's holes means
                // the hazard is nested inside the candidate's pocket -> NOT colliding.
                let p = haz_shape.poi.center;
                let in_outer = shape.collides_with(&p);
                if in_outer && shape_holes.iter().any(|h| h.collides_with(&p)) {
                    false
                } else {
                    in_outer
                }
    ...
    pub fn collect_poly_collisions(&self, shape: &SPolygon, collector: &mut impl HazardCollector) {
        self.collect_poly_collisions_with_holes(shape, &[], collector);
    }

    /// Like [`Self::collect_poly_collisions`] but accounts for the candidate's own holes.
    pub fn collect_poly_collisions_with_holes(
        &self,
        shape: &SPolygon,
        shape_holes: &[Arc<SPolygon>],
        ...
        //Collect all colliding entities due to boundary segment intersection
        shape
            .segment_iter()
            .for_each(|segment| v_quadtree.collect_collisions(&segment, collector));
...
pub struct CDEConfig {
    ...
    /// Stop traversing the quadtree and perform collision collection immediately when the total number of boundary segments in a node falls below this number
    pub cd_threshold: u8,
...
mod tests {
    ...
    fn tessellated_shape(shape: &SPolygon) -> SPolygon {
        let points = shape
            .tessellated_edge_iter(0.001)
            .map(|edge| edge.start)
            .collect_vec();
    ...
    fn oracle_queries() -> Vec<SPolygon> {
        vec![
            rect_query(-0.05, 2.90, 0.05, 3.05),
            rect_query(-0.25, 1.25, 0.25, 1.75),
            rect_query(2.50, 2.50, 2.75, 2.75),
            rect_query(-1.25, -0.25, -0.95, 0.25),
            rect_query(0.25, -1.25, 0.75, -0.95),
        ]
    }

    fn engine_with_inner_hazard(shape: SPolygon) -> CDEngine {
        let outer =
            std::sync::Arc::new(SPolygon::from(Rect::try_new(-5.0, -5.0, 5.0, 5.0).unwrap()));
        let hazard = std::sync::Arc::new(shape);
        CDEngine::new(
            outer.bbox.inflate_to_square(),
            vec![
                Hazard::new(HazardEntity::Exterior, outer, false),
                Hazard::new(
                    HazardEntity::InferiorQualityZone { quality: 0, idx: 0 },
                    hazard,
                    false,
                ),
            ],
    ...
    #[test]
    fn detect_poly_collision_uses_arc_segments_in_registered_hazards() {
        let curved_hazard = rounded_top_shape();
        let chord_hazard = SPolygon::new(curved_hazard.vertices.clone()).unwrap();
        let query = SPolygon::from(Rect::try_new(-0.05, 2.90, 0.05, 3.05).unwrap());

        let chord_edges_collide = chord_hazard.edge_iter().any(|haz_edge| {
            query
                .edge_iter()
                .any(|query_edge| query_edge.collides_with(&haz_edge))
        });
        assert!(!chord_edges_collide);
        assert!(!engine_with_inner_hazard(chord_hazard).detect_poly_collision(&query, &NoFilter));
        assert!(engine_with_inner_hazard(curved_hazard).detect_poly_collision(&query, &NoFilter));
    }

    #[test]
    fn curved_cde_matches_fine_tessellation_oracle() {
        for curved_hazard in [rounded_top_shape(), stadium_shape()] {
            let tessellated_hazard = tessellated_shape(&curved_hazard);
            let curved_engine = engine_with_inner_hazard(curved_hazard);
            let tessellated_engine = engine_with_inner_hazard(tessellated_hazard);

            for query in oracle_queries() {
                assert_eq!(
                    curved_engine.detect_poly_collision(&query, &NoFilter),
                    tessellated_engine.detect_poly_collision(&query, &NoFilter),
                    "curved CDE result diverged from tessellation oracle for query {:?}",
                    query.vertices
                );
            }
        }
