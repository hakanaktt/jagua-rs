...
pub struct QTNode {
    ...
    /// Stop traversing the quadtree and perform collision detection immediately when the total number of active boundary segments in a node falls below this number
    pub cd_threshold: u8,
}
...
impl QTNode {
    ...
    pub fn collect_collisions<T: QTQueryable>(
        ...
        // Condition to perform collision detection now or pass it to children:
        let perform_cd_now = self.hazards.n_active_segments() <= self.cd_threshold as usize;
