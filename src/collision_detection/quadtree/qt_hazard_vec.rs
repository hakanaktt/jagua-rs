...
pub struct QTHazardVec {
    ...
    /// Number of boundary segments from active hazards in the vector
    n_active_segments: usize,
}
...
impl QTHazardVec {
    ...
    pub fn add(&mut self, haz: QTHazard) {
        ...
        {
            Ok(pos) | Err(pos) => {
                self.n_active_segments += haz.n_segments();
    ...
    pub fn n_active_segments(&self) -> usize {
        debug_assert!(assert_caches_correct(self));
        self.n_active_segments
    }
}
...
fn assert_caches_correct(qthazard_vec: &QTHazardVec) -> bool {
    ...
    assert_eq!(
        qthazard_vec
            .hazards
            .iter()
            .map(|hz| hz.n_segments())
            .sum::<usize>(),
        qthazard_vec.n_active_segments,
        "Active boundary segment count is not correct!"
    );
    true
}
