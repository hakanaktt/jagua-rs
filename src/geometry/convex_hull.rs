...
/// Returns hull points using the polygon vertices plus arc extrema from curved boundary segments.
pub fn convex_hull_points(shape: &SPolygon) -> Vec<Point> {
    convex_hull_from_points(shape.boundary_points())
}
...
pub fn convex_hull_from_surrogate(s: &SPolygon) -> Result<Vec<Point>> {
    if let Some(surr) = s.surrogate.as_ref() {
        Ok(surr.convex_hull_points.clone())
    } else {
