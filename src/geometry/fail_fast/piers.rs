...
fn clip(shape: &SPolygon, ray: &Edge) -> Vec<Edge> {
    ...
    let intersections = shape
        .segment_iter()
        .flat_map(|segment| segment.collides_at(ray))
...
mod tests {
    ...
    #[test]
    fn clip_uses_arc_boundary_intersections() {
        let shape = rounded_top_shape();
        let ray = Edge::try_new(Point(0.0, -1.0), Point(0.0, 4.0)).unwrap();

        let clips = clip(&shape, &ray);

        assert_eq!(clips.len(), 1);
        assert!(clips[0].start.1 < 0.01);
        assert!(clips[0].end.1 > 2.99);
    }
}
